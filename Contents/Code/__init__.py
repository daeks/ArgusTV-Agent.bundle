        ...
        try:
          ...
          if isinstance(tmdb_dict, dict) and 'results' in tmdb_dict:
            for i, movie in enumerate(sorted(tmdb_dict['results'], key=lambda k: k['popularity'], reverse=True)):
              ...
              Log('[ARGUS] Matched movie %s against IMDB %s' % (root_file, id))
          
          if len(results) == 0:
            results.Append(MetadataSearchResult(id = media.id, name = media.name, year = None, score = 100, lang = Prefs['language']))
            Log('[ARGUS] No IMDB match found for movie %s' % root_file)
          else:
            results.Append(MetadataSearchResult(id = media.id, name = media.name, year = None, score = 100, lang = Prefs['language']))
            Log('[ARGUS] No IMDB match found for movie %s' % root_file)
        except:
         Log('[ARGUS] Could not retrieve data from PLEX for: %s' % root_file)
         results.Append(MetadataSearchResult(id = media.id, name = media.name, year = None, score = 100, lang = Prefs['language']))
      else:
        results.Append(MetadataSearchResult(id = media.id, name = media.name, year = None, score = 100, lang = Prefs['language']))
        Log('[ARGUS] No metadata file available for %s' % root_file)
    else:
      results.Append(MetadataSearchResult(id = media.id, name = media.name, year = None, score = 100, lang = Prefs['language']))
  ...
  def update(self, metadata, media, lang):
    ...
    if os.path.isfile(os.path.join(path, root_file + '.thmb')):
      ...
      if media_hash not in metadata.art:
        ...
        Log('[ARGUS] Thumbnail added for %s' % metadata.id)
    ...
    if os.path.isfile(os.path.join(path, root_file + '.arg')):
      ...
      for fname in os.listdir(path):
        if fname.endswith(Prefs['format']):
          i = i +1
      if i > 1:
        metadata.collections.add(metadata.title)
        Log('[ARGUS] Set collection for %s: %s - Count: %i' % (root_file, metadata.title, i))
      ...
      if Prefs['tmdb']:
        try:
          if metadata.id.isdigit():
            Log('[ARGUS] Lookup Poster for IMDB ID: %s' % metadata.id)
            ...
            Log('[ARGUS] No IMDB ID: %s' % metadata.id)
        except:
         Log('[ARGUS] Could not retrieve data from PLEX for: %s' % metadata.title)
...
def imdb_id_from_tmdb(tmdb_id):
  ...
  try:
    ...
    Log('[ARGUS] Error: Cannot get imdb id from tmdb id (%s) - %s' % (tmdb_id, str(e)))
